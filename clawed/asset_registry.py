...
class AssetRegistry:
    ...
    def search_assets(
        ...
        with sqlite3.connect(self._db_path) as conn:
            ...
            if teacher_id:
                rows = conn.execute(
                    "SELECT * FROM assets WHERE teacher_id = ?",
                    (teacher_id,),
                ).fetchall()
            else:
    ...
    def stats(self, teacher_id: str) -> dict[str, int]:
        ...
        with sqlite3.connect(self._db_path) as conn:
            total = conn.execute(
                "SELECT COUNT(*) FROM assets WHERE teacher_id = ?",
                (teacher_id,),
            ).fetchone()[0]
            images = conn.execute(
                "SELECT COUNT(*) FROM asset_images ai JOIN assets a ON ai.asset_id = a.id WHERE a.teacher_id = ?",
                (teacher_id,),
            ).fetchone()[0]
            links = conn.execute(
                "SELECT COUNT(*) FROM asset_links al JOIN assets a ON al.asset_id = a.id WHERE a.teacher_id = ?",
                (teacher_id,),
            ).fetchone()[0]
