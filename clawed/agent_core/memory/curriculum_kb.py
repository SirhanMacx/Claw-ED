...
class CurriculumKB:
    ...
    def _init_db(self) -> None:
        with sqlite3.connect(self._db_path) as conn:
            ...
            """)
            conn.execute("CREATE INDEX IF NOT EXISTS idx_chunks_teacher ON chunks(teacher_id)")
            conn.execute("CREATE UNIQUE INDEX IF NOT EXISTS idx_chunks_dedup ON chunks(teacher_id, chunk_hash)")
        ...
        for batch_start in range(0, len(chunks), self._INDEX_BATCH):
            ...
            with sqlite3.connect(self._db_path) as conn:
                for chunk in batch:
                    ...
                    try:
                        conn.execute(
                            "INSERT INTO chunks_fts(rowid, chunk_text, doc_title) VALUES (?, ?, ?)",
    ...
    def _apply_quality_boost(results: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        lesson_plan_re = re.compile(
            r"\b(Do\s+Now|Exit\s+Ticket|Objective|Aim)\b",
            re.IGNORECASE,
        )
        vocab_re = re.compile(
            r"\b(vocabulary|key\s+terms)\b",
            re.IGNORECASE,
        )
    ...
    def _search_numpy(
        ...
        max_dim = max(max(len(e) for e in embeddings), len(q))
        if len(q) < max_dim:
