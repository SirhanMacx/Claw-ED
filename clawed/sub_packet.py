...
async def generate_sub_packet(
    ...
    topic_line = f"Lesson Topic: {request.lesson_topic}\n" if request.lesson_topic else ""
    context_line = f"Unit/Context: {request.lesson_context}\n" if request.lesson_context else ""
...
def sub_packet_to_markdown(packet: SubPacket) -> str:
    ...
    lines.append("---")
    lines.append(f"*Generated by Claw-ED on {packet.generated_at.strftime('%Y-%m-%d %H:%M')}*")
    return "\n".join(lines)
