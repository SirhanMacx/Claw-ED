...
const OUT = join(APP, 'app-store/screenshots/ipad-12.9');
...
async function setCards(page, state) {
  await page.evaluate((state) => {
    ...
    if (state.connect) byId('connect-card').hidden = false;
    if (state.input !== undefined) byId('server-url').value = state.input;
    if (state.error) {
...
try {
  ...
  await screenshot(page, '01-pair-from-mac.png', 'iPad companion', 'Scan the Mac QR code and open your classroom workspace.');
  ...
  console.log(JSON.stringify({ ok: true, outDir: OUT, screenshots: 5, size: '2048x2732' }, null, 2));
} finally {
